...
import { ITenancy } from "../../../../server/types/interfaces";
...
export async function createTenancy(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.log(documents, "FROM REQ BODY");

    let uploadedPDFArr = await Promise.all(
      documents.map((PDF: string) => {
        return uploadImage(PDF, "unit-documents");
      })
    );
    uploadedPDFArr.map((PDF) => {
      return PDF.secure_url();
    });
    console.log(uploadedPDFArr, "THE RETURNED PDF ARR");
