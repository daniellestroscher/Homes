...
export default function RentRoll({ unitArr, allVersions, colorMode }: Props) {
  ...
  function createRows(): readonly Row[] {
    ...
    unitArr.forEach((unit) => {
      if (unit.tenancies && unit.tenancies[0] && unit.tenancies[0].tenants) {
        ...
        let currentTenancy = unit.tenancies.find(
          (one) => one.activeStatus === true
        );
        let tenant = `${
          currentTenancy &&
          currentTenancy.tenants &&
          currentTenancy.tenants[0].lastName
        }`;
        let rents: rentsType[] = [];
        unit.tenancies.forEach((tenancy) => {
          //find versions
          let tenancyRents = allVersions.filter(
            (version) => tenancy.id === version.id
          );
  ...
  const gridElement = (
    <DataGrid
      ...
      className={
        colorMode === "default" ? "rdg-light fill-grid" : "rdg-dark fill-grid"
      }
