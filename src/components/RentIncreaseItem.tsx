...
export default function RentIncreaseItem({ community, unit, tenancy, selector }: Props) {
  ...
  if (
    new Date(tenancy?.establishedDate as string).getTime() >
    new Date().getTime()
  ) {
    established = false;
  }
  ...
  return (
    <a
      href={selector ? "javascript:void(0)" : `/${id}/${unit.id}`}
    >
      <div
        ...
        {tenancy && tenancy.tenancy_versions && (
          <div sx={{ position: "absolute", bottom: "5px", left: "5px" }}>
            {established ? (
              ...
              <h6 sx={{ color: "darkRed" }}>
                This tenancy <br /> is not yet established.
              </h6>
            )}
          </div>
        )}
