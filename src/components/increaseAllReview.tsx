...
export default function IncreaseAll({ unitList }: Props) {
  ...
  const [increasePercent, setIncreasePercent] = useState<number | undefined>(
    undefined
  );
  const [effectiveDate, setEffectiveDate] = useState<string>(
    formatDate(formatRentEffectiveDate(new Date()), "yyyy-mm-dd")
  );
    ...
    if (increasePercent) {
      unitList.forEach((unit) => {
        if (
          ...
          unit.tenancies[0].tenancy_versions[0].rent &&
          increasePercent
        ) {
          let rent = unit.tenancies[0].tenancy_versions[0].rent;
          let increaseDiff =
            Number(rent.toString().slice(1)) * (increasePercent / 100);
          ...
          rentIncreaseObjArr.push({
            id: unit.tenancies[0].id as string,
      ...
      await Promise.all(
        rentIncreaseObjArr.map((version) => {
          createRentIncrease({
            id: version.id,
                  ...
                  >
                    <h4>
                      {"#" +
                        unit.number +
                        " " +
                        unit.tenancies[0].tenants[0].lastName}
                    </h4>
